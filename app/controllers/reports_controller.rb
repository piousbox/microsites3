...
class ReportsController < ApplicationController
  ...
  def create
    @report = Report.new params[:report].permit( :name, :subhead, :descr, :venue, :city, :x, :y, :tag, :is_public, :photo )
  ...
  def update
    ...
    respond_to do |format|
      if @report.update_attributes(params[:report].permit( :name, :subhead, :descr, :venue, :city, :x, :y, :tag, :is_public, :photo ))
        format.html do
  ...
  def show
    unless params[:name_seo].blank?
      @report = Report.unscoped.where( :name_seo => params[:name_seo] ).first
    else
      @report = Report.unscoped.find params[:id]
    end
