class GalleriesController < ApplicationController
  ...
  def show
    ...
    if params[:id]
      ...
      if @gallery = Gallery.unscoped.where( :galleryname => params[:galleryname] ).first
        ...
        if @gallery.tag.blank?
          ...
          @related_galleries = @related_galleries[0..2]
        end
  ...
  def update
    @g = Gallery.unscoped.find params[:id]
    authorize! :update, @g

    if @g.update_attributes params[:gallery].permit( :username, :name, :galleryname, :subhead, :descr, :lang  )
      flash[:notice] = 'Success'
    else
      flash[:error] = 'No Luck'
      @cities_list = City.list
