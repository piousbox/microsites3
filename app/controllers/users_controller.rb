...
class UsersController < ApplicationController
  ...
  def organizer
    ...
    @my_reports = Report.unscoped.where({ :user => current_user, :site => @site, :is_trash => false }).order_by({ :created_at => :desc }).page( params[:reports_page] )
  ...
  def update
    ...
    if @user.update_attributes params[:user].permit( :scratchpad, :github_path, :facebook_path, :stackoverflow_path, :name, :display_ads,
                                                     :dispplay_help, :profile_photo, :guide_city, :current_city )
  ...
  def create_profile
    ...
    @user_profile = UserProfile.new params[:user_profile].permit( :about, :education, :objectives, :current_employment, :past_employment,
                                                                  :pdf_resume_path, :doc_resume_path, :lang )
  ...
  def update_profile
    ...
    if @user_profile.update_attributes params[:user_profile].permit( :about, :education, :objectives, :current_employment, :past_employment,
                                                                     :pdf_resume_path, :doc_resume_path, :lang )
      flash[:notice] = 'Success'
      redirect_to organizer_path
    else
