...
describe UsersController, :type => :controller do
  ...
  describe 'reports' do
    it 'should show reports' do
      ...
      get :reports, :username => @user.username, :locale => :en
    ...
    it 'should show one report' do
      get :report, :name_seo => @user.reports.first.name_seo, :username => @user.username, :locale => :en
      response.should be_success
    ...
    it 'not found' do
      get :report, :name_seo => 'Nonexistant report.', :username => 'anon', :locale => :en
      response.should be_success
  ...
  describe 'galleries' do
    it 'should list galleries' do
      get :galleries, :username => @user.username, :locale => :en
      response.should be_success
    ...
    it 'should show gallery' do
      ...
      get :gallery, :username => @user.username, :galleryname => @user.galleries.first.galleryname, :locale => :en
      response.should be_success
  ...
  describe 'github page' do
    it 'should show the github page inside piousbox' do
      sign_out :user
      get :github_page, :username => @user.username, :locale => :en
      response.should be_success
  ...
  describe 'profiles' do
    ...
    it 'creates profile. sets current user as the owner of the profile when creating' do
      ...
      post :create_profile, :user_profile => profile, :locale => :en
    ...
    it 'GETs edit profile' do
      ...
      get :edit_profile, :username => @user.username, :profile_id => @user.user_profiles[0].id, :locale => :en
      response.should be_success
    ...
    it '#update_profile' do
      @user.user_profiles << UserProfile.new( :lang => :en, :about => 'A little about me' )
      @user.save.should eql true
      put :update_profile, :profile_id => @user.user_profiles[0].id,
           :user_profile => { :about => 'the new about' }, :locale => :en, :username => @user.username
  ...
  describe 'report' do
    it 'should GET report of a user' do
      get :reports, :username => @user.username, :name_seo => @r1.name_seo, :locale => :en
      response.should render_template('users/reports')
    end
  end
  ...
  describe 'settings' do
    ...
    it 'should let edit user' do
      ...
      post :update, :user => user, :id => @user.id, :locale => :en
      result = User.find @user.id
  ...
  it 'GETs show' do
    get :show, :username => @user.username, :locale => :en
    response.should render_template( 'users/show' )
  end

  describe 'messages' do
    it 'sends a new message' do
      post :new_message, :locale => :en
      response.should be_redirect
