...
describe PhotosController do
  ...
  describe 'create' do
    it 'should save logged in' do
      ...
      descr = '24twebfvsdfg'
      photo = { :descr => descr }
      post :create, :photo => photo, :locale => :en
    ...
    it 'puts the photo in a gallery based on params' do
      post :create, :photo => @photo_attrs, :galleryname => @gallery.galleryname, :locale => :en
    ...
    it 'GETs multinew' do
      get :multinew, :galleryname => @gallery.galleryname, :locale => :en
    ...
    it 'should not save without recaptcha' do
      expect_any_instance_of(PhotosController).to receive(:verify_recaptcha).and_return(false)
    ...
    it '#j_create' do
      ...
      post :j_create, :photo => @photo_attrs, :locale => :en, :format => :json, :galleryname => @gallery.galleryname
      ( Photo.all.length - old_n ).should eql 1
    end

    it 'should save with recaptcha' do
      expect_any_instance_of(PhotosController).to receive(:verify_recaptcha).and_return(true)
      
      sign_out :user
      n_old = Photo.all.length
      
      post :create, :photo => @photo_attrs, :locale => :en
      n_new = Photo.all.length
      ( n_new - n_old ).should eql 1
    end
    ...
    it 'should add a newsitem to the sites - en, ru, pt' do
      ...
      post :create, :photo => { :descr => 'blah blah', :is_public => true, :create_newsitems => 1 }, :locale => :en
    ...
    it 'should not add a newsitem to the site, if the box is unchecked' do
      ...
      post :create, :photo => { :descr => 'blah blah', :is_public => true, :create_newsitems => 0 }, :locale => :en
  ...
  describe 'new profile photo' do
    ...
    it 'should post new profile photo' do
      ...
      post :create, :photo => { :set_as_profile_photo => true, :name => 'aaa' }, :locale => :en
  ...
  describe 'new photo' do
    it 'should GET new photo for a gallery for anon' do
      ...
      assert_template :new_for_gallery
    end
  ...
  describe '#destroy' do
    ...
    it 'does' do
      ...
      @ph.user = @user
      @ph.save
      delete :destroy, :id => @ph.id, :format => :json, :locale => :en
